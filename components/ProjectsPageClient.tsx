          ...
          <div className="flex items-center gap-2">
            {userEmail && (
              <div className="flex items-center gap-2 mr-2">
                <div className="w-7 h-7 rounded-full bg-[#D9822B] flex items-center justify-center text-white text-xs font-semibold">
                  {userEmail.slice(0, 2).toUpperCase()}
                </div>
                <span className="text-sm text-[#6B6F76]">{userEmail}</span>
              </div>
            )}
            {isAdmin && (
            ...
            {isAdmin && (
              <Link href="/admin/users" className="flex items-center gap-1.5 px-4 py-2 border border-[#E8E8E9] text-sm text-[#151B26] rounded-md hover:bg-[#F5F5F5]">
                <Users size={14} /> Users
              </Link>
            )}
            {isAdmin && (
              <Link href="/projects/new" className="flex items-center gap-1.5 px-4 py-2 bg-[#4573D9] text-white text-sm rounded-md hover:bg-[#3F65C4]">
